...
// => full
const queue1 = cinemaQueue(6, ['X','O','X','O','O','X','X','X','O','X']);
const queue1_F = cinemaQueueFIFO(6, ['X','O','X','O','O','X','X','X','O','X']);
console.log(queue1, queue1_F)
// => 'too much: 2'
const queue2 = cinemaQueue(6, ['X','O','X','O','O','X','X','X','O','X','X','X']);
const queue2_F = cinemaQueueFIFO(6, ['X','O','X','O','O','X','X','X','O','X','X','X']);
...
function camelCase(n) {
  return n.trim().split(' ').map((char, i) => {
    const firstLetter = (i == 0) ? char.charAt(0).toLowerCase() : char.charAt(0).toUpperCase();
